...
\- Phase 105.2.8.1 fixes the nullable callback type for the disabled low-energy Quick Care Play action.

\- `_PetQuickAction.onTap` now correctly accepts `VoidCallback?`, allowing the Play action to pass `null` below the 8% Energy threshold.

\- Removes the resulting analyzer error, unnecessary-null-comparison warning, and dead-code warnings from the disabled-state styling added in Phase 105.2.8.



**### Fixed**

\- Phase 105.2.8 improves low-energy Play handling for shared pets.

\- Play now becomes unavailable when Energy is below 8%, matching the existing service rule instead of allowing a Firestore transaction to surface a wrapped Flutter Web Future exception.

\- Pet Detail shows a clear `Too tired` state and tells the user to Feed the pet first to restore energy.

\- Chat Quick Care uses the same low-energy Play rule and disabled styling.

\- Feed remains available and restores 7 Energy, while the service-level Energy validation remains in place as a safety guard.



**### Fixed**

\- Phase 105.2.7 adds an explicit Material surface around the floating-pet action sheet used by View pet details, Quick care, and Hide pet.

\- Fixes Flutter Web debug exceptions reporting `ListTile background color or ink splashes may be invisible` when opening the floating-pet menu.

\- Existing floating-pet suppression while Shared Pet Detail is open, saved visibility preference, dragging, position persistence, and PetActor behavior are unchanged.



**### Fixed**

\- Phase 105.2.6 temporarily hides the chat floating pet while Shared Pet Detail is open.

\- Returning to the chat automatically restores the floating pet when the user's saved Show pet preference is enabled.

\- Temporary pet-page suppression is kept separate from the persisted Hide pet / Show pet preference, so opening Pet Detail never changes the user's visibility setting.

\- Existing floating-pet position persistence, dragging, root Overlay architecture, and PetActor appearance are unchanged.
...
\- Cleared the remaining \`curly\_braces\_in\_flow\_control\_structures\` lint in the direct chat page.
...
\- Phase 104.7.2 replaces the corrupted Phase 104.7.1 \`pet\_actor.dart\` with a clean full-file implementation.
...
\- Removed the old \`mochi\_floating.png\` single-image asset path.
...
\- Fixed 7 analyzer errors in \`real\_chat\_room\_page.dart\`.
...
\- Group chat now marks \`group\_\<groupId>\` as the active conversation so the device currently viewing that group is not needlessly notified.
...
\- Added \`shared\_preferences\` for device-local appearance persistence.

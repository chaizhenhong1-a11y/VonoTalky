...
typedef TreeTagAuthorNameStream = Stream<String> Function(String authorId);

typedef TreeTagDeleteCallback =
    Future<void> Function({required String treeOwnerId, required String tagId});
...
class TreeTagLayer extends StatefulWidget {
  ...
  final Future<void> Function(TreeLeafTag tag)? onCreateTag;
...
class _TreeTagLayerState extends State<TreeTagLayer> {
  ...
  void initState() {
    ...
    _tags = List<TreeLeafTag>.from(widget.initialTags);
  }

  @override
  void didUpdateWidget(covariant TreeTagLayer oldWidget) {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final localViewportHeight = constraints.maxHeight;
        ...
        final fullViewportHeight =
            localViewportHeight + widget.contentTopOffset;
        ...
        final painterHeight = fullViewportHeight + 50.0;

        /*
 * 必须和 Painter 的 _getTreeScale()
 * 使用完全相同的算法。
 */
        final widthScale = viewportWidth / 390.0;

        final heightScale = painterHeight / 800.0;
 ...
 */
        final crownCenterGlobalY =
            sceneTop + painterHeight * 0.72 - 300 * treeScale;

        final crownCenterLocalY = crownCenterGlobalY - widget.contentTopOffset;
 ...
 * radius = 150 * scale
 */
        final crownRadius = 150.0 * treeScale;

        final crownWidth = crownRadius * 2;

        final crownHeight = crownRadius * 2;

        final crownLeft = viewportWidth / 2 - crownRadius;

        final crownTop = crownCenterLocalY - crownRadius;

        final localCrownPath = _buildLocalCrownPath(radius: crownRadius);
  ...
  Path _buildLocalCrownPath({required double radius}) {
    /*
  ...
  Future<void> _handleTreeTap({
    ...
    if (!crownPath.contains(position)) {
      return;
    }
    ...
    final relativeX = (position.dx / crownWidth).clamp(0.0, 1.0).toDouble();

    final relativeY = (position.dy / crownHeight).clamp(0.0, 1.0).toDouble();

    final content = await _showWriteTagSheet();
    ...
    final tag = TreeLeafTag(
      ...
      treeOwnerId: widget.treeOwnerId,
    ...
    try {
      if (widget.onCreateTag != null) {
        ...
        widget.onTagsChanged?.call(List<TreeLeafTag>.unmodifiable(_tags));
      }
    } catch (error) {
      if (!mounted) {
        return;
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('标签没有挂成功，请稍后再试')));
    }
  }
  ...
  Widget _buildTag({
    ...
    final x = crownLeft + crownWidth * tag.x;

    final y = crownTop + crownHeight * tag.y;
  ...
  Future<void> _openTag(TreeLeafTag tag) async {
    final canDelete =
    ...
    final deleted = await showModalBottomSheet<bool>(
      ...
      builder: (sheetContext) {
        return _TreeTagDetailSheet(
          tag: tag,
          authorNameStream: widget.watchAuthorName?.call(tag.authorId),
          canDelete: canDelete && widget.onDeleteTag != null,
...
class _TreeTagView extends StatelessWidget {
  const _TreeTagView({required this.tag, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    final rotation = ((tag.id.hashCode % 9) - 4) * 0.012;
...
class _WriteTreeTagSheetState extends State<_WriteTreeTagSheet> {
        ...
        child: SafeArea(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                '给这棵树的主人留下一句话。',
                style: TextStyle(fontSize: 14, color: Color(0xFF918A7D)),
              ),
      ...
      child: SafeArea(
        ...
        child: Column(
          ...
          children: [
            ...
            const Row(
              children: [
                Icon(Icons.local_offer_rounded, color: Color(0xFF8B642E)),
                SizedBox(width: 10),
                Text(
                  '树叶上的标签',
                  style: TextStyle(
            ...
            Row(
              children: [
                ...
                if (widget.authorNameStream != null)
                  ...
                  const Text(
                    '来自 VonoTalky 用户',
                    style: TextStyle(fontSize: 13, color: Color(0xFF797267)),
                  ),
              ],
            ),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: const Color(0x33764F25)),
              ),
              child: Text(
                widget.tag.content,
                style: const TextStyle(
            ...
            Text(
              ...
              style: const TextStyle(fontSize: 12, color: Color(0xFF9B9386)),
            ),
            if (widget.canDelete) ...[
              const SizedBox(height: 20),
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  ...
                  label: Text(_deleting ? '正在删除…' : '删除这张标签'),
                ),
              ),
            ],
          ],

...
Future<void> showFriendTreeListSheet(BuildContext context) async {
  ...
  await Navigator.of(context).push(
    MaterialPageRoute<void>(
      builder: (context) {
        return FriendTreePage(user: friend);
      },
    ),
  );
}
...
class FriendTreeListSheet extends StatefulWidget {
  const FriendTreeListSheet({super.key});
...
class _FriendTreeListSheetState extends State<FriendTreeListSheet> {
            ...
            children: [
              ...
              Expanded(
                child: StreamBuilder<List<ChatUser>>(
                  stream: _contactService.contacts(),
                  builder: (context, snapshot) {
                    ...
                    final friends = snapshot.data ?? const <ChatUser>[];
...
class _FriendTreeTile extends StatelessWidget {
  const _FriendTreeTile({required this.friend, required this.onTap});
...
class _FriendAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final firstCharacter = friend.name.trim().isEmpty
        ? '?'
      ...
      child: photoUrl != null && photoUrl.isNotEmpty
          ? Image.network(
              ...
              errorBuilder: (context, error, stackTrace) {
                return _AvatarFallback(text: firstCharacter);
              },
            )
          : _AvatarFallback(text: firstCharacter),
    );
  }
}
...
class _EmptyFriendTrees extends StatelessWidget {
...
class _FriendTreeError extends StatelessWidget {
  const _FriendTreeError({required this.onRetry});
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.cloud_off_outlined, size: 42, color: colorScheme.error),

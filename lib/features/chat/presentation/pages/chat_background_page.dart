...
class _ChatBackgroundPageState extends State<ChatBackgroundPage> {
  final ChatBackgroundService _service = ChatBackgroundService();
  ...
  Future<void> _chooseBackground() async {
    ...
    try {
      await _service.pickAndUpload(widget.otherId);

      if (!mounted) return;

      _notice('Background updated for this chat');
      ...
      _notice('Background could not be uploaded');
    } finally {
      if (mounted) {
        setState(() {
          uploading = false;
  ...
  Future<void> _setShared(bool value) async {
    ...
    try {
      await _service.setShared(otherId: widget.otherId, shared: value);
      ...
      _notice('Sharing setting could not be changed');
    } finally {
      if (mounted) {
        setState(() {
          changingShare = false;
  ...
  Future<void> _removeBackground() async {
    ...
    try {
      await _service.remove(widget.otherId);

      if (!mounted) return;

      _notice('Background removed');
    } catch (_) {
      if (!mounted) return;

      _notice('Background could not be removed');
    } finally {
      if (mounted) {
        setState(() {
          removing = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<ChatBackgroundInfo>(
        stream: _service.watchMine(widget.otherId),
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return const Center(
              child: Text('Background settings could not be loaded'),
            );
          }

          final info =
              snapshot.data ??
              const ChatBackgroundInfo(imageUrl: null, shared: false);
            ...
            children: [
              ...
              SizedBox(
                ...
                child: FilledButton.icon(
                  onPressed: uploading || removing ? null : _chooseBackground,
                  icon: uploading
                      ? const SizedBox(
                          width: 18,
                          height: 18,
              ...
              if (info.hasImage) ...[
                ...
                SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    onPressed: uploading || removing ? null : _removeBackground,
                    icon: removing
                        ? const SizedBox(
                            width: 17,
                            height: 17,
                    ...
                    label: Text(removing ? 'Removing...' : 'Remove background'),
                  ),
                ),
              ],
...
class _BackgroundPreview extends StatelessWidget {
  const _BackgroundPreview({required this.imageUrl, required this.otherName});
            ...
            if (imageUrl != null)
              Image.network(
                ...
                errorBuilder: (context, error, stackTrace) {
                  return _DefaultBackground(colors: colors);
                },
              )
            else
              _DefaultBackground(colors: colors),
              ...
              child: Column(
                children: [
                  ...
                  const Align(
                    ...
                    child: _PreviewBubble(text: 'Hey 👋', mine: false),
                  ),
                  ...
                  const Align(
                    ...
                    child: _PreviewBubble(
                      text: 'I changed our chat background.',
                      mine: true,
                    ),
                  ),
                    ...
                    child: const Row(
                      children: [
                        Expanded(
                          child: Text(
                            'Type a message...',
                            style: TextStyle(color: Color(0xFF827A86)),
                          ),
                        ),
...
class _DefaultBackground extends StatelessWidget {
  const _DefaultBackground({required this.colors});
...
class _PreviewBubble extends StatelessWidget {
  const _PreviewBubble({required this.text, required this.mine});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(text, style: const TextStyle(color: Color(0xFF28232C))),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: SwitchListTile(
        value: shared,
        onChanged: hasImage && !loading ? onChanged : null,

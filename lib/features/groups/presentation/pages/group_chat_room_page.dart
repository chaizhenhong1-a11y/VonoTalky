...
class _GroupChatRoomPageState extends State<GroupChatRoomPage> {
  ...
  Future<void> _pickImage(ImageSource source) async {
    ...
    try {
      final upload = await mediaService.upload(
        file: file,
  ...
  Future<void> _pickFile() async {
    ...
    setState(() => uploading = true);
    try {
      final upload = await fileService.upload(
        file: file,
        roomId: widget.group.id,
        userId: service.myId,
        root: 'group_media',
  ...
  Future<void> _editGroupMessage(
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .collection('messages')
          .doc(messageId)
          .update({'text': updated, 'editedAt': FieldValue.serverTimestamp()});
